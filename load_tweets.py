...
def insert_tweet(connection,tweet):
    ...
    with connection.begin() as trans:
        ...
        res = connection.execute(sql,{
              'id_users' : remove_null(tweet['user']['id'])
            , 'created_at' : remove_null(tweet['user']['created_at'])
            , 'updated_at' :  remove_null(tweet['created_at'])
            ...
            , 'screen_name' : remove_null(tweet['user']['screen_name'])
            , 'name' : remove_null(tweet['user']['name'])
            , 'location' : remove_null(tweet['user']['location'])
            , 'description' : remove_null(tweet['user']['description'])
            , 'withheld_in_countries' :  remove_null(tweet['user'].get('withheld_in_countries', []))
            }
            )
